...
def train():
    ...
    for epoch in range(epoch):
        ...
        print("Test acc: {0}".format(correct.item() /
                                 len(test_dataset)))
        torch.save(obj=net.state_dict(), f="models/2MNIST_2000.pth")#准确率0.97899
    torch.save(obj=net.state_dict(), f="models/2MNIST_2000.pth")

def test():
    batch_size = 8
    epoch = 10
    dataset = PicDataset("./pic/methods2/triggertest")
    train_size = int(len(dataset) * 0.7)
    test_size = len(dataset) - train_size
    train_dataset, test_dataset = torch.utils.data.random_split(dataset, [train_size, test_size])
    ...
    net = WaterMarkedNet().to(device)
    net = net.double()
    net.load_state_dict(torch.load("models/2MNIST_1000.pth"))
    net.eval()  # 将模型变换为测试模式
    correct = 0
    total = 0
    for data_test in test_loader:
